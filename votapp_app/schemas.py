...
class SurveyCreate(BaseModel):
    ...
    # Segmentación como listas
    sexo: List[str] = Field(default_factory=list)
    ciudad: List[str] = Field(default_factory=list)
    ocupacion: List[str] = Field(default_factory=list)
    profesion: List[str] = Field(default_factory=list)   # 👈 nuevo campo
    nivel_educativo: List[str] = Field(default_factory=list)
    religion: List[str] = Field(default_factory=list)
    nacionalidad: List[str] = Field(default_factory=list)
    estado_civil: List[str] = Field(default_factory=list)
...
# -------------------
# Billetera (Eliminado pro estar duplicado)
# -------------------
...
class SurveyWalletOut(BaseModel):
    ...
    description: Optional[str] = None
    fecha_creacion: Optional[datetime] = None
    fecha_expiracion: Optional[datetime] = None   # 👈 debe llegar como datetime, no string
    presupuesto_total: Optional[int] = None
    visibilidad_resultados: Optional[str] = None
    recompensa_puntos: Optional[int] = None
    recompensa_dinero: Optional[int] = None
    patrocinada: Optional[bool] = None
    patrocinador: Optional[str] = None
    ...
    questions: Optional[List[PreguntaOut]] = None
    media_url: Optional[str] = None
    media_urls: Optional[List[str]] = None

    # 👇 obligatorio, no opcional
    usuario_id: int = Field(..., description="ID del usuario dueño de la encuesta")
