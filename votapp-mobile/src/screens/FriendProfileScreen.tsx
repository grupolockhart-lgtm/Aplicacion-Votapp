...
type User = {
  ...
  otro_usuario_id?: number;   // 👈 agrega este campo
...
export default function FriendProfileScreen({ route }: FriendProfileProps) {
  ...
  const fetchUser = async () => {
    try {
      const res = await fetch(`${API_URL}/usuarios/${friendId}?current_user_id=${currentUserId}`);
  ...
  const fetchFriendGamificacion = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/gamificacion/usuarios/${friendId}/gamificacion`,
        { headers: { Authorization: `Bearer ${storedToken}` } }
      );
      const data = await res.json();
  ...
  const sendFriendRequest = async () => {
    ...
    setSending(true);
    try {
      await fetch(`${API_URL}/friends/request?user_id=${currentUserId}&friend_id=${user.id}`, { method: "POST" });
  ...
  const acceptFriendRequest = async (friendshipId: number) => {
    try {
      await fetch(`${API_URL}/friends/${friendshipId}?action=accepted`, { method: "PUT" });
  ...
  const rejectFriendRequest = async (friendshipId: number) => {
    try {
      await fetch(`${API_URL}/friends/${friendshipId}?action=rejected`, { method: "PUT" });
    ...
    Alert.alert("Confirmar", "¿Seguro que quieres eliminar esta amistad?", [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            const storedToken = await AsyncStorage.getItem("userToken");
            await fetch(`${API_URL}/friends/${friendshipId}`, { method: "DELETE", headers: { Authorization: `Bearer ${storedToken}` } });
  ...
  return (
    <View style={styles.container}>
      ...
      {user.status === "pending" && user.friendship_id ? (
        user.role === "received" ? (
          // Recibida: mostrar Aceptar/Rechazar
