...
import { API_URL, endpoints } from "@/config/api"; // 👈 centralizado
...
export default function NotificationsScreen() {
  ...
  const fetchUnreadCount = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/notifications/unread_count?user_id=${userId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await res.json();
  ...
  const acceptFriendRequest = async (friendshipId: number, notificationId: number) => {
    if (!token) return;
    try {
      const res = await fetch(`${endpoints.friends}/${friendshipId}?action=accepted`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
  ...
  const rejectFriendRequest = async (friendshipId: number, notificationId: number) => {
    if (!token) return;
    try {
      await fetch(`${endpoints.friends}/${friendshipId}?action=rejected`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
      });
